...
public class TERenderer {
    ...
    public void renderFrame(TETile[][] world, boolean isLightOn, int avatarX, int avatarY) {
        ...
        if (isLightOn) {
            ...
            for (int x = 0; x < width; x++) {
                for (int y = 0; y < height; y++) {
                    tempWorld[x][y] = isWithinLightRadius(x, y, avatarX, avatarY) ? world[x][y] : Tileset.NOTHING;
                    tempWorld[x][y].draw(x + xOffset, y + yOffset);
                }
            }
        }
    }
    ...
    private boolean isWithinLightRadius(int x, int y, int centerX, int centerY) {
        return x > centerX - LIGHT_RADIUS && x < centerX + LIGHT_RADIUS && y > centerY - LIGHT_RADIUS && y < centerY + LIGHT_RADIUS;
    }
}
